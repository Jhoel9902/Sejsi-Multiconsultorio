...
spCancelar: BEGIN
    ...
    IF v_estado_cita <> 'confirmada' THEN
        SET p_mensaje = 'Solo se pueden cancelar citas confirmadas';
        LEAVE spCancelar;
    END IF;
    ...
    -- Eliminar facturas de cliente relacionadas a esta cita
    DELETE FROM tfactura_cliente
    WHERE id_cita = p_id_cita;

    -- Eliminar facturas de aseguradora relacionadas a esta cita
    DELETE FROM tfactura_aseguradora
    WHERE id_cita = p_id_cita;

    SET p_success = TRUE;
    SET p_mensaje = 'Cita cancelada exitosamente y facturas eliminadas';
...
spCrear: BEGIN
    ...
    IF NOT EXISTS(SELECT 1 FROM tpaciente WHERE id_paciente = p_id_paciente AND estado = 1) THEN
        SET p_success = FALSE;
        SET p_mensaje = 'Paciente no encontrado o inactivo';
        LEAVE spCrear;
    END IF;
    ...
    IF NOT EXISTS(SELECT 1 FROM tpersonal WHERE id_personal = p_id_personal AND estado = 1) THEN
        SET p_success = FALSE;
        SET p_mensaje = 'Médico no encontrado o inactivo';
        LEAVE spCrear;
    END IF;
    ...
    IF NOT EXISTS(
        ...
        SET p_success = FALSE;
        SET p_mensaje = 'La hora no está disponible para este médico en esta fecha';
        LEAVE spCrear;
    END IF;
...
spMarcarAsistencia: BEGIN
    DECLARE v_estado_cita VARCHAR(30);
    DECLARE v_pagada INT DEFAULT 0;
    ...
    IF v_estado_cita IS NULL THEN
        SET p_mensaje = 'Cita no encontrada';
        LEAVE spMarcarAsistencia;
    END IF;
    
    -- Validar que esté confirmada
    IF v_estado_cita != 'confirmada' THEN
        SET p_mensaje = 'Solo se pueden completar citas confirmadas';
        LEAVE spMarcarAsistencia;
    END IF;

    -- Validar que la cita esté pagada (debe existir factura de cliente o aseguradora con pago registrado)
    SELECT COALESCE(
        (SELECT 1 FROM tfactura_cliente 
         WHERE id_cita = p_id_cita AND estado = 1 AND metodo_pago IS NOT NULL
         LIMIT 1),
        (SELECT 1 FROM tfactura_aseguradora 
         WHERE id_cita = p_id_cita AND estado = 1
         LIMIT 1),
        0
    ) INTO v_pagada;
...
spReprogramar: BEGIN
    ...
    IF v_estado_cita != 'confirmada' THEN
        SET p_mensaje = 'Solo se pueden reprogramar citas confirmadas';
        LEAVE spReprogramar;
    END IF;
    
    -- Validar que no haya sido reprogramada antes (máximo 1 reprogramación)
    IF v_nro_reprogramaciones >= 1 THEN
        SET p_mensaje = 'Esta cita ya fue reprogramada. No se permite más de 1 reprogramación';
        LEAVE spReprogramar;
    END IF;
    
    -- Validar que la fecha no sea anterior a hoy
    IF p_fecha_nueva < CURDATE() THEN
        SET p_mensaje = 'La nueva fecha no puede estar en el pasado';
        LEAVE spReprogramar;
    END IF;
    ...
    IF NOT EXISTS(
        ...
        SET p_mensaje = 'La hora no está disponible para este médico en esa fecha';
        LEAVE spReprogramar;
    END IF;
    
    -- Validar que no exista otra cita en ese horario (excepto la actual) - excluir canceladas
    ...
    AND estado_cita IN ('confirmada', 'en_atencion', 'completada')
    AND estado = 1;
    
    IF v_existe_cita > 0 THEN
        SET p_mensaje = 'El horario ya está ocupado';
        LEAVE spReprogramar;
    END IF;
...
BEGIN
    ...
    -- Marcar la cita como pagada (cambiar estado_cita a 'confirmada' con pago registrado)
    -- Se mantendrá en 'confirmada' hasta que se marque asistencia
    UPDATE tcita
...
DELIMITER //
CREATE PROCEDURE `sp_pac_buscar`(
...
CREATE TABLE IF NOT EXISTS `tcita` (
  ...
  PRIMARY KEY (`id_cita`),
  UNIQUE KEY `UX_cita_medico_fecha_hora` (`id_personal`,`fecha_cita`,`hora_cita`,`estado_cita`),
...
INSERT INTO `tcita` (`id_cita`, `id_paciente`, `id_personal`, `id_servicio`, `fecha_cita`, `hora_cita`, `motivo_consulta`, `observaciones`, `estado_cita`, `nro_reprogramaciones`, `motivo_cancelacion`, `estado`, `fecha_creacion`, `fecha_actualizacion`) VALUES
	('0194e23f-deb3-11f0-8c44-40c2ba62ef61', '8d40d2d7-deb2-11f0-8c44-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222011', '2025-12-22', '10:00:00', 'prueba', NULL, 'completada', 0, NULL, 1, '2025-12-21 17:21:25', '2025-12-21 17:31:43'),
	...
	('17458c36-de56-11f0-955d-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', '3910b567-da7f-11f0-9321-40c2ba62ef61', '22222222-2222-2222-2222-222222222020', '2025-12-22', '11:30:00', 'asd', NULL, 'confirmada', 0, NULL, 1, '2025-12-21 06:16:18', NULL),
	('2fe273bd-debb-11f0-8c44-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', '2025-12-22', '10:15:00', 'denuevo cita en hora de cita cancelada antes', NULL, 'completada', 0, NULL, 1, '2025-12-21 18:19:59', '2025-12-21 18:27:09'),
	...
	('4615d0ea-dcd3-11f0-b3a3-40c2ba62ef61', '7f0a6ea8-da06-11f0-90da-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222012', '2025-12-23', '09:00:00', 'asd', NULL, 'confirmada', 0, NULL, 1, '2025-12-19 08:07:22', NULL),
	('52279710-dc8a-11f0-924c-40c2ba62ef61', 'af470af8-d8fd-11f0-8c16-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', '2025-12-22', '15:30:00', 'asd', NULL, 'completada', 0, NULL, 1, '2025-12-18 23:25:09', '2025-12-21 18:52:05'),
	('9796705c-dc89-11f0-924c-40c2ba62ef61', '7f0a48bb-da06-11f0-90da-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', '2025-12-22', '15:00:00', 'asd', NULL, 'cancelada', 0, 'se retracto', 1, '2025-12-18 23:19:56', '2025-12-21 18:55:25'),
	('97d010dd-dcd2-11f0-b3a3-40c2ba62ef61', 'ab57d017-d864-11f0-9531-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222012', '2025-12-23', '07:00:00', 'asd', NULL, 'confirmada', 0, NULL, 1, '2025-12-19 08:02:29', NULL),
	('9ad273cb-dcd1-11f0-b3a3-40c2ba62ef61', '7f0a6ea8-da06-11f0-90da-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', '2025-12-22', '15:45:00', 'prueba factura aseguradora', NULL, 'cancelada', 0, 'pete', 1, '2025-12-19 07:55:25', '2025-12-21 19:04:35'),
	('9b6aa943-dce1-11f0-807f-02503ee1e8bc', 'ab57fd94-d864-11f0-9531-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222011', '2025-12-23', '08:00:00', 'd', NULL, 'completada', 0, NULL, 1, '2025-12-19 09:49:58', '2025-12-19 09:53:33'),
	('baaff83c-dec1-11f0-8c44-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222011', '2025-12-23', '08:00:00', 'prueba aseguradora', NULL, 'cancelada', 0, 'nose', 1, '2025-12-21 19:06:49', '2025-12-21 19:10:22'),
	('cf0a1971-deba-11f0-8c44-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', '2025-12-22', '10:15:00', 'prueba cancelar superpuesto', NULL, 'cancelada', 0, 'probemos', 1, '2025-12-21 18:17:16', '2025-12-21 18:19:09'),
	('df38a56c-dec3-11f0-8c44-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222021', '2025-12-22', '10:00:00', 'asd', NULL, 'confirmada', 0, NULL, 1, '2025-12-21 19:22:09', NULL),
	('e9d5a606-dcdf-11f0-807f-02503ee1e8bc', '7f0a4d72-da06-11f0-90da-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', '22222222-2222-2222-2222-222222222021', '2025-12-22', '16:00:00', 'consulta', NULL, 'cancelada', 0, 'pendeja', 1, '2025-12-19 09:37:50', '2025-12-21 19:03:40'),
	('f51c56c1-de55-11f0-955d-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', '3910b567-da7f-11f0-9321-40c2ba62ef61', '22222222-2222-2222-2222-222222222020', '2025-12-22', '15:00:00', 'nolo se', NULL, 'confirmada', 0, NULL, 1, '2025-12-21 06:15:21', NULL);
...
INSERT INTO `tdetalle_factura_cliente` (`id_detalle_cliente`, `id_factura_cliente`, `id_servicio`, `descripcion`, `cantidad`, `precio_unitario`, `estado`, `fecha_creacion`) VALUES
	('276b6f07-debc-11f0-8c44-40c2ba62ef61', '276a0e64-debc-11f0-8c44-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', NULL, 1, 220.00, 1, '2025-12-21 18:26:54'),
	('a1fa28ae-debf-11f0-8c44-40c2ba62ef61', 'a1f99248-debf-11f0-8c44-40c2ba62ef61', '22222222-2222-2222-2222-222222222010', NULL, 1, 220.00, 1, '2025-12-21 18:51:48'),
	('d11c756f-deb3-11f0-8c44-40c2ba62ef61', 'd11bc4e1-deb3-11f0-8c44-40c2ba62ef61', '22222222-2222-2222-2222-222222222011', NULL, 1, 280.00, 1, '2025-12-21 17:27:13');
...
INSERT INTO `testudio` (`id_estudio`, `id_historial`, `id_personal`, `nombre_estudio`, `foto`, `fecha_subida`, `estado`) VALUES
	('05d88798-de59-11f0-955d-40c2ba62ef61', 'cab47f8f-de57-11f0-955d-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', 'asda', '/uploads/estudios/estudio-1766313437993-16261.pdf', '2025-12-21 06:37:18', 1),
	('aa5e26cd-de58-11f0-955d-40c2ba62ef61', 'cab47f8f-de57-11f0-955d-40c2ba62ef61', 'e8ad4426-da87-11f0-81e7-40c2ba62ef61', 'geneologia ', '/uploads/estudios/estudio-1766313284520-46331.jpg', '2025-12-21 06:34:44', 1);
...
INSERT INTO `tfactura_cliente` (`id_factura_cliente`, `id_cita`, `id_paciente`, `fecha_emision`, `numero_factura`, `subtotal`, `total`, `metodo_pago`, `observaciones`, `estado`, `fecha_creacion`) VALUES
	('276a0e64-debc-11f0-8c44-40c2ba62ef61', '2fe273bd-debb-11f0-8c44-40c2ba62ef61', 'c77409c2-da88-11f0-81e7-40c2ba62ef61', '2025-12-21 18:26:54', 'FAC-CL-2025-00002', 220.00, 220.00, 'efectivo', NULL, 1, '2025-12-21 18:26:54'),
	('a1f99248-debf-11f0-8c44-40c2ba62ef61', '52279710-dc8a-11f0-924c-40c2ba62ef61', 'af470af8-d8fd-11f0-8c16-40c2ba62ef61', '2025-12-21 18:51:48', 'FAC-CL-2025-00003', 220.00, 220.00, 'tarjeta_debito', NULL, 1, '2025-12-21 18:51:48'),
	('d11bc4e1-deb3-11f0-8c44-40c2ba62ef61', '0194e23f-deb3-11f0-8c44-40c2ba62ef61', '8d40d2d7-deb2-11f0-8c44-40c2ba62ef61', '2025-12-21 17:27:13', 'FAC-CL-2025-00001', 280.00, 280.00, 'efectivo', NULL, 1, '2025-12-21 17:27:13');
...
INSERT INTO `thistorial_paciente` (`id_historial`, `id_paciente`, `id_personal`, `id_cita`, `diagnosticos`, `evoluciones`, `antecedentes`, `tratamientos`, `fecha_creacion`, `fecha_ultima_actualizacion`, `estado`) VALUES
	('8d4272cd-deb2-11f0-8c44-40c2ba62ef61', '8d40d2d7-deb2-11f0-8c44-40c2ba62ef61', NULL, NULL, '', '', '', '', '2025-12-21 17:18:10', NULL, 1),
	('cab47f8f-de57-11f0-955d-40c2ba62ef61', 'cab397f5-de57-11f0-955d-40c2ba62ef61', NULL, NULL, '', '', '', '', '2025-12-21 06:28:29', NULL, 1);
...
INSERT INTO `tpaciente` (`id_paciente`, `nombre`, `apellido_paterno`, `apellido_materno`, `fecha_nacimiento`, `ci`, `estado_civil`, `domicilio`, `nacionalidad`, `tipo_sangre`, `alergias`, `contacto_emerg`, `enfermedad_base`, `observaciones`, `celular`, `correo`, `codigo_paciente`, `estado`, `fecha_creacion`, `fecha_actualizacion`) VALUES
	...
	('8d40d2d7-deb2-11f0-8c44-40c2ba62ef61', 'pepillo', 'validacion', NULL, '2021-10-29', '4512365S', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '+591 7775577', NULL, 'PAC-20251221-28774', 0, '2025-12-21 17:18:10', '2025-12-21 17:21:34'),
	...
	('ab57deb1-d864-11f0-9531-40c2ba62ef61', 'Sofía', 'Rivera', 'Blanco', '2000-08-08', '9012345', 'Soltera', 'Av. 6 de Agosto #2345, Irpavi', 'Boliviana', 'A+', 'Frutos secos', 'Ricardo Cruz - 77234489', 'Ninguna', 'Estudiante universitaria', '59179234567', 'sofia.rivera@email.com', 'PAC-20241201-00901', 1, '2025-12-13 16:45:33', NULL),
	...
	('ab57ebe2-d864-11f0-9531-40c2ba62ef61', 'Mario', 'Guzmán', 'Vega', '1960-11-28', '2233445', 'Casado', 'Av. Circunvalación #789, 3er Anillo', 'Boliviana', 'A+', 'Aspirina', 'Claudia Zeballos - 76234478', 'Parkinson, Osteoporosis', 'Cuidado especial, movilidad reducida', '59176234478', 'mario.guzman@email.com', 'PAC-20241201-01345', 1, '2025-12-13 16:45:33', NULL),
	...
	('cab397f5-de57-11f0-955d-40c2ba62ef61', 'juanito ', 'historioa', NULL, '2014-12-31', '7451248A', NULL, NULL, NULL, 'A+', NULL, NULL, NULL, NULL, '84512487', NULL, 'PAC-20251221-30566', 1, '2025-12-21 06:28:29', NULL);
...
INSERT INTO `tpaciente_aseguradora` (`id_paciente`, `id_aseguradora`, `numero_poliza`, `fecha_asignacion`, `fecha_fin`, `estado`) VALUES
	('ab57c083-d864-11f0-9531-40c2ba62ef61', '68f65393-da8e-11f0-81e7-40c2ba62ef61', 'asdasd', '2025-12-21 17:24:34', NULL, 1),
	('ab5802dd-d864-11f0-9531-40c2ba62ef61', 'e52b5710-da13-11f0-81c4-40c2ba62ef61', 'asdasd', '2025-12-21 17:23:39', NULL, 1),
	('c77409c2-da88-11f0-81e7-40c2ba62ef61', 'e52b5aca-da13-11f0-81c4-40c2ba62ef61', 'plosadas', '2025-12-21 19:06:02', NULL, 1);
...
INSERT INTO `tpersonal` (`id_personal`, `ci`, `nombres`, `apellido_paterno`, `apellido_materno`, `cargo`, `id_rol`, `fecha_nacimiento`, `fecha_contratacion`, `domicilio`, `celular`, `correo`, `contrasena`, `foto_perfil`, `archivo_contrato`, `estado`, `fecha_creacion`, `fecha_actualizacion`) VALUES
	...
	('69a37493-d87f-11f0-81b0-40c2ba62ef61', '6784411', 'Ignacio', 'Bocangel', NULL, 'Supervisor', '0fe2336b-d854-11f0-9531-40c2ba62ef61', '2004-05-16', '2014-08-21', 'Sopocachi', '74561511', 'Ignacio@gmail.com', '$2a$10$ZeP0iKiN/Y9VBLKQ/PC/x.eJNMWCfY3QRyOBLdNv92F6L6YEnK9MO', '/uploads/personal/fotos/foto-1766150904414-567421685.jpg', '/uploads/personal/contratos/contrato-1765670219137-392840608.pdf', 1, '2025-12-13 19:56:59', '2025-12-19 09:28:24'),
	...
	('b9f16541-deb3-11f0-8c44-40c2ba62ef61', '3211234', 'ventanilla', 'aperl', NULL, '', '0fe2393a-d854-11f0-9531-40c2ba62ef61', '2002-12-31', '2025-12-31', NULL, '+59174812458', 'vent@vent.com', '$2a$10$f54U4UJH277K4HTOsZmkyusikESapxiP3WpUB8p50zbtPncychtjm', NULL, NULL, 1, '2025-12-21 17:26:34', NULL),
	('c082da3a-de54-11f0-955d-40c2ba62ef61', '2233879', 'testcincer', 'nose', NULL, '', '0fe2393a-d854-11f0-9531-40c2ba62ef61', '1998-12-01', '2025-01-01', NULL, '+59174812844', NULL, '$2a$10$SfKEptkycmL5H4iYe/0RmuV8RKeWDJAY4JHwed/r234T0Mji8IyUG', NULL, NULL, 0, '2025-12-21 06:06:43', '2025-12-21 06:42:04'),
...
INSERT INTO `tpersonal_especialidad` (`id_personal`, `id_especialidad`, `fecha_asignacion`, `estado`) VALUES
	...
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '11111111-1111-1111-1111-111111111010', '2025-12-19 09:29:47', 1);
...
INSERT INTO `tpersonal_horario` (`id_personal`, `id_horario`, `dia_descanso`, `fecha_asignacion`, `estado`) VALUES
	('3910b567-da7f-11f0-9321-40c2ba62ef61', '213f9e37-dc1b-11f0-bfba-40c2ba62ef61', 'Miércoles', '2025-12-21 06:14:58', 1),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f87f8-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-18 10:34:11', 1),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f8905-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-18 10:33:34', 1),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f8b52-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-18 23:00:01', 0),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f96a5-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-19 09:30:59', 1),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f9e37-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-21 17:20:44', 1),
	('e8ad4426-da87-11f0-81e7-40c2ba62ef61', '213f9e4e-dc1b-11f0-bfba-40c2ba62ef61', 'Domingo', '2025-12-18 17:33:14', 1);
...
INSERT INTO `tservicio` (`id_servicio`, `nombre`, `precio`, `descripcion`, `fecha_creacion`, `estado`) VALUES
	...
	('cd259e68-dec2-11f0-8c44-40c2ba62ef61', 'piña2', 950.00, 'ya no te dan una piña', '2025-12-21 19:14:29', 1);
